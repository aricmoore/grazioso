...
public class Driver {
	...
	private static void intakeNewMonkey(Scanner scanner) {
		...
		// Restricts input to enum
		String species = RescueAnimalHelper.promptForSpecies(scanner);
  ...
  //   boolean found = false;
	...
	// 	AnimalType type;
	// 	try {
	// 		type = AnimalType.fromString(typeInput);
  ...
  //   }
    ...
    if (!found) {
      System.out.printf("No available %ss found in %s.%n", animalType.toString(), country);
    }
	}
    ...
    for (RescueAnimal animal : animalList) {
			if (!animal.getReserved() && animal.getInServiceCountry().equalsIgnoreCase(country)) {
				...
				String animalTypeFormatted = animalType.substring(0, 1).toUpperCase() + animalType.substring(1).toLowerCase();
