...
public class Driver {
	...
	public static void intakeNewDog(Scanner scanner) {
		...
		newDog.setName(scanner.nextLine().trim());
    ...
    if (animalType.equals("dog")) {
        for (Dog dog : dogList) {
            // Check if dog is not reserved and in the requested country
            if (!dog.getReserved() && dog.getInServiceCountry().equalsIgnoreCase(country)) {
                ...
                found = true;
                break;  // Only reserve the first matching animal
            }
        }
    } else if (animalType.equals("monkey")) {
        for (Monkey monkey : monkeyList) {
            // Check if monkey is not reserved and in the requested country
