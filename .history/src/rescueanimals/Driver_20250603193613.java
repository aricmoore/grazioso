...
public class Driver {
	...
	public static void printAnimals(String input) {
		switch (input) {
			...
			case "6":
				...
				// Loop through both lists and print only animals that aren't reserved
				System.out.println("\nAvailable Dogs:");
				int dogCount = 0;
				for (Dog dog : dogList) {
					if (!dog.getReserved() && dog.getTrainingStatus() == RescueAnimal.TrainingStatus.IN_SERVICE) {
						System.out.printf("Name: %s, Status: %s, Acquisition Country: %s, Reserved: %s%n",
							dog.getName(),
				...
				if (dogCount == 0) {
					System.out.println("No available dogs found.");
				}

				System.out.println("\nAvailable Monkeys:");
				int monkeyCount = 0;
				for (Monkey monkey : monkeyList) {
					if (!monkey.getReserved() && monkey.getTrainingStatus() == RescueAnimal.TrainingStatus.IN_SERVICE) {
						System.out.printf("Name: %s, Status: %s, Acquisition Country: %s, Reserved: %s%n",
							monkey.getName(),
            ...
            foundAvailable = true;
				...
				if (!foundAvailable) {
					System.out.println("\nNo available animals match the criteria.");
				}
				break;
			default:
	...
	public static void printAvailableAnimalsByType(String animalType) {
		...
		// Prints the available animal list in a formatted table with necessary data
